...
// POST con body { nombre: string, b64: string, tipo_archivo?: string }
// Auth: x-user-email debe ser admin.
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Actualizar formatos_plantillas con la URL
    ...
    if (updErr) {
      log.warn("Storage OK pero formatos_plantillas update fallo", { nombre, err: updErr.message });
      return NextResponse.json({ url, warning: "tabla no actualizada: " + updErr.message });
    }
