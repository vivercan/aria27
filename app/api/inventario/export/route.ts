...
// ─── PDF Galeria: grid de fotos grandes (P-5 modo galeria) ──
    ...
    return `
      <div class="card">
        <div class="card-img-wrap">
          <img src="${escHtml(item.foto_url || "")}" alt="${escHtml(item.producto_nombre)}" class="card-img" onerror="this.style.display='none'" />
        </div>
        <div class="card-body">
...
<head>
<meta charset="UTF-8" />
<title>ARIA27 — Galeria ${escHtml(obraNombre)}</title>
<style>
...
<body>
  <div class="header">
    <h1>Galeria de Inventario - ${escHtml(obraNombre)}</h1>
    <div class="meta">${conFoto.length} productos con foto${sinFoto ? ` (${sinFoto} sin foto)` : ""} - ${escHtml(generadoEn)}</div>
  </div>
  <div class="grid">${cards || '<p style="color:#475569;font-size:11pt">No hay productos con foto en este inventario.</p>'}</div>
  <div class="footer">
    <span>ARIA27 ERP - Grupo Constructor Urbano Avante - Usuario: ${escHtml(email)}</span>
    <span>${escHtml(generadoEn)}</span>
  </div>
</body>
</html>`;
