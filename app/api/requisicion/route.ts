...
// Obtener usuario por ROL (con manejo de errores)
async function getUserByRole(role: string) {
  try {
    ...
    if (error) {
      console.error(`Error buscando rol ${role}:`, error.message);
      return null;
    }
    
    if (!data || data.length === 0) {
      console.error(`No se encontro usuario con rol: ${role}`);
      return null;
    }
    
    console.log(`Usuario ${role} encontrado:`, data[0].email);
    return data[0];
  } catch (e: any) {
    console.error(`Excepcion buscando rol ${role}:`, e.message);
    return null;
  }
}
  ...
  try {
    ...
    try {
      await resend.emails.send({
        ...
        html: `<div style="font-family:Arial;max-width:650px;margin:0 auto;border-radius:2px;overflow:hidden"><div style="background:linear-gradient(135deg,#0f172a,#1e3a5f);color:white;padding:25px;text-align:center"><div style="font-size:28px;font-weight:900;letter-spacing:2px;color:#22d3ee">ARIA</div><div style="font-size:10px;text-transform:uppercase;color:#94a3b8;letter-spacing:3px;margin-top:4px">Operations OS</div><p style="margin:8px 0 0;font-size:16px;opacity:0.9">ARIA27 ERP</p></div><div style="padding:25px"><h2 style="color:#1e3a5f">Requisicion Generada</h2><p>Hola <strong>${displayName}</strong>, tu requisicion ha sido registrada${isValidador ? " y validada automaticamente" : ""}.</p><div style="background:#f8fafc;border-radius:8px;padding:20px;margin:20px 0"><table style="width:100%"><tr><td style="color:#64748b">Folio:</td><td style="font-weight:bold">${folio}</td></tr><tr><td style="color:#64748b">Obra:</td><td style="font-weight:bold">${obra}</td></tr><tr><td style="color:#64748b">Generada:</td><td>${fechaGen}</td></tr><tr><td style="color:#64748b">Requerida:</td><td style="font-weight:bold;color:${urgencyColor}">${fechaReq}</td></tr></table></div>${tablaHtml}</div>${emailFooter}</div>`
      });
    ...
    // 3. EMAIL AL ADMIN (si no es quien creo)
    if (!isAdmin && adminUser) {
      ...
      if (adminUser.phone) { await sendWhatsAppTemplate("requisicion_creada", [folio, displayName, obra, fechaReq], adminUser.phone); }
      notificados.push(`Admin: ${adminUser.email}`);
    }

    // 4. SI ES VALIDADOR, DIRECTO A COMPRAS
    if (isValidador && comprasUser) {
      ...
      if (comprasUser.phone) { await sendWhatsAppTemplate("requisicion_compras", [folio, obra, urgencyText], comprasUser.phone); }
