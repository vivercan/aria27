...
const ESTATUS_OPTIONS = [
  ...
  { value: "actualizado", label: "Actualizado", color: "bg-emerald-500/20 text-aria-accent" },
];
    ...
    <div className="h-full flex flex-col overflow-hidden">
      <div className="flex items-center justify-between mb-4 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-2xl font-bold text-white">SIROC / Registro IMSS</h1><p className="text-xs text-[#7f93b0]">{registros.length} registros</p></div>
        </div>
      ...
      <div className="flex items-center gap-3 mb-3 flex-shrink-0">
        <div className="relative flex-1 max-w-xs"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[#4a6080]" /><input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar registro, bimestre..." className="w-full pl-9 pr-3 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-sm focus:border-aria-primary focus:outline-none placeholder-slate-600" /></div>
      ...
      <div className="flex-1 overflow-y-auto rounded-xl bg-white/[0.02] border border-white/[0.06]">
        <table className="w-full">
          <thead className="sticky top-0 bg-[rgba(4,8,16,0.98)]  z-10"><tr className="border-b border-white/[0.08]">
            <th className="text-left p-3 text-[#7f93b0] font-medium text-xs">Registro</th>
            <th className="text-left p-3 text-[#7f93b0] font-medium text-xs">Obra</th>
            <th className="text-center p-3 text-[#7f93b0] font-medium text-xs">Bimestre</th>
            <th className="text-center p-3 text-[#7f93b0] font-medium text-xs">Trabajadores</th>
            <th className="text-center p-3 text-[#7f93b0] font-medium text-xs">Incidencias</th>
          ...
          <tbody>
            ...
            ) : filtrados.length === 0 ? (<tr><td colSpan={7} className="p-8 text-center"><ClipboardList className="w-10 h-10 text-[#4a6080] mx-auto mb-2" /><p className="text-[#4a6080] text-sm">{registros.length === 0 ? "No hay registros" : "Sin resultados"}</p></td></tr>
            ) : filtrados.map(s => (
              <tr key={s.id} className="border-b border-white/[0.05] hover:bg-white/[0.02]">
                <td className="p-3"><p className="text-white text-sm font-medium">{s.numero_registro}</p><p className="text-[#4a6080] text-xs">{s.responsable || "â"}</p></td>
                <td className="p-3 text-[#7f93b0] text-sm">{s.obra_nombre || "â"}</td>
                ...
                <td className="p-3 text-center"><span className={`text-xs px-2 py-0.5 rounded-full ${getEstatus(s.estatus)?.color || "bg-slate-500/20 text-[#7f93b0]"}`}>{getEstatus(s.estatus)?.label || s.estatus}</span></td>
