...
const ESTATUS_OPTIONS = [
  ...
  { value: "registrado", label: "Registrado", color: "bg-blue-500/20 text-blue-400" },
...
export default function SIROCPage() {
  ...
  const guardar = async () => {
    ...
    if (form.file) {
      ...
      const path = `siroc/${form.obra_id}/${timestamp}.${ext}`;
    ...
    const payload: any = {
      ...
      documento_url: urlFile, responsable: form.responsable?.trim() || null,
      observaciones: form.observaciones?.trim() || null,
    };

    if (editId) {
      const { error } = await supabase.from("siroc").update(payload).eq("id", editId);
      if (error) msg("error", error?.message ?? "Error"); else { msg("success", "Registro actualizado"); setShowForm(false); setEditId(null); cargar(); }
    } else {
      const { error } = await supabase.from("siroc").insert(payload);
      if (error) msg("error", error?.message ?? "Error"); else { msg("success", "Registro registrado"); setShowForm(false); cargar(); }
    }
    setGuardando(false);
  };
  ...
  const confirmDelete = async () => {
    try { await backupAndDelete({ table: "siroc", id: deleteModal.id, userEmail }); msg("success", "Eliminado"); } catch (e: any) { msg("error", e?.message || "Error"); }
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      <div className="flex items-center justify-between mb-4 flex-shrink-0">
        ...
        <button onClick={() => { setShowForm(true); setEditId(null); setForm({ ...EMPTY_FORM }); }} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 text-white text-sm hover:bg-blue-700"><Plus className="w-4 h-4" /> Nuevo Registro</button>
      </div>

      <div className="grid grid-cols-3 gap-3 mb-4 flex-shrink-0">
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3"><p className="text-blue-400 text-2xl font-bold">{totalRegistros}</p><p className="text-blue-400/70 text-xs">Total Registros</p></div>
      ...
      <div className="flex items-center gap-3 mb-3 flex-shrink-0">
        <div className="relative flex-1 max-w-xs"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" /><input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar registro, bimestre..." className="w-full pl-9 pr-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm focus:border-blue-500 focus:outline-none placeholder-slate-600" /></div>
            ...
            {loading ? (<tr><td colSpan={7} className="p-8 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto text-blue-400" /></td></tr>
              ...
              <tr key={s.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                ...
                <td className="p-3 text-center"><div className="flex items-center justify-center gap-1">
                  <button onClick={() => editar(s)} className="p-1.5 rounded-lg bg-blue-500/20 text-blue-400 hover:bg-blue-500/30"><Edit2 className="w-3.5 h-3.5" /></button>
