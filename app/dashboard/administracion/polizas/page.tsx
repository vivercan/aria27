    ...
    <div className="aria-bg-canon h-full flex flex-col overflow-hidden">
      <div className="flex items-center justify-between mb-4 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-2xl font-bold text-white">Pólizas de Seguro</h1><p className="text-xs text-[#7f93b0]">{polizas.length} pólizas registradas</p></div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4 flex-shrink-0">
        <div className="bg-aria-primary/10 border border-aria-primary/20 rounded-xl p-3"><p className="text-white text-2xl font-bold">{totalPolizas}</p><p className="text-aria-accent/70 text-xs">Total Pólizas</p></div>
        <div className="bg-emerald-500/10 border border-white/[0.08] rounded-xl p-3"><p className="text-white text-2xl font-bold">{vigentes}</p><p className="text-aria-accent/70 text-xs">Vigentes</p></div>
      ...
      <div className="flex items-center gap-3 mb-3 flex-shrink-0">
        <div className="relative flex-1 max-w-xs"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[#4a6080]" /><input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar póliza, aseguradora..." className="w-full pl-9 pr-3 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-sm focus:border-aria-primary focus:outline-none placeholder-slate-600" /></div>
      ...
      <div className="flex-1 overflow-y-auto rounded-xl bg-white/[0.02] border border-white/[0.06]">
        <table className="w-full">
          <thead className="sticky top-0 bg-[rgba(4,8,16,0.98)]  z-10"><tr className="border-b border-white/[0.08]">
            <th className="text-left p-3 text-[#7f93b0] font-medium text-xs">Póliza</th>
            ...
            <th className="text-left p-3 text-[#7f93b0] font-medium text-xs">Aseguradora</th>
          ...
          <tbody>
            {loading ? (<tr><td colSpan={7} className="p-8 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto text-aria-accent" /></td></tr>
            ) : filtrados.length === 0 ? (<tr><td colSpan={7} className="p-8 text-center"><Shield className="w-10 h-10 text-[#4a6080] mx-auto mb-2" /><p className="text-[#4a6080] text-sm">{polizas.length === 0 ? "No hay pólizas registradas" : "Sin resultados"}</p></td></tr>
            ) : filtrados.map(p => {
              ...
              <tr key={p.id} className="border-b border-white/[0.05] hover:bg-white/[0.02]">
                <td className="p-3"><p className="text-white text-sm font-medium">{p.numero_poliza}</p><p className="text-[#4a6080] text-xs">{p.cobertura || "â"}</p></td>
                ...
                <td className="p-3 text-[#7f93b0] text-sm">{p.aseguradora}</td>
                ...
                <td className="p-3 text-center"><span className={`text-xs px-2 py-0.5 rounded-full ${getEstatus(estatusActual)?.color || "bg-slate-500/20 text-[#7f93b0]"}`}>{getEstatus(estatusActual)?.label || estatusActual}</span></td>
        ...
        <div className="fixed inset-0 bg-black/60  z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-[#0f1729] border border-white/[0.08] rounded-2xl w-full max-w-lg max-h-[85vh] overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 space-y-3 overflow-y-auto max-h-[60vh]">
              ...
              <div><label className="block text-xs text-[#7f93b0] mb-1">Contacto</label><input type="text" value={form.contacto} onChange={e => setForm({ ...form, contacto: e.target.value })} placeholder="Nombre, teléfono" className={inputClass} /></div>
