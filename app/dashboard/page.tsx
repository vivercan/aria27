  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (storedEmail) {
          const { data: user, error: UsersErr } = await supabase
            .from("Users")
            .select("display_name, name")
            .eq("email", storedEmail)
            .single();
          if (UsersErr) console.error("Error loading Users:", UsersErr.message);
          if (user) {
        ...
        const { count: reqHoy, error: RequisicionesErr3 } = await supabase
          .from("Requisiciones")
          .select("*", { count: "exact", head: true })
          .gte("created_at", today);
        if (RequisicionesErr3) console.error("Error loading Requisiciones:", RequisicionesErr3.message);

        // Requisiciones pendientes
        const { count: reqPend, error: RequisicionesErr2 } = await supabase
          .from("Requisiciones")
          .select("*", { count: "exact", head: true })
        ...
        const { count: reqApproved, error: RequisicionesErr } = await supabase
          .from("Requisiciones")
          .select("*", { count: "exact", head: true })
          .eq("status", "AUTORIZADA");
        if (RequisicionesErr) console.error("Error loading Requisiciones:", RequisicionesErr.message);

        // Empleados activos
        const { count: emps, error: PersonalErr } = await supabase
          .from("Personal")
          .select("*", { count: "exact", head: true })
          .eq("status", "ACTIVO");
        ...
        // Productos
        const { count: prods, error: ProductosErr } = await supabase
          .from("Productos")
          .select("*", { count: "exact", head: true });
        if (ProductosErr) console.error("Error loading Productos:", ProductosErr.message);

        // Centros de costo
        const { count: centers, error: centrosTrabajoerr } = await supabase
          .from("centros_trabajo")
          .select("*", { count: "exact", head: true });
        if (centrosTrabajoerr) console.error("Error loading centros_trabajo:", centrosTrabajoerr.message);
