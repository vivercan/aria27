...
export default function ClockInPage() {
  ...
  async function fetchAsistencias() {
    ...
    const { data } = await supabase
      .from("asistencias")
      .select("*, employees(employee_number, full_name, position)")
      .eq("fecha", fecha)
  ...
  const filtered = asistencias.filter(a =>
    a.employees?.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    a.employees?.employee_number?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <Link href="/dashboard/talent-hub" className="inline-flex items-center gap-2 text-slate-400 hover:text-white text-sm">
        ← Regresar al Panel
      </Link>
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                ...
                <tr key={a.id} className="hover:bg-white/5">
                  <td className="px-4 py-3 text-blue-300 font-mono text-xs">{a.employees?.employee_number}</td>
                  <td className="px-4 py-3 text-white">{a.employees?.full_name}</td>
