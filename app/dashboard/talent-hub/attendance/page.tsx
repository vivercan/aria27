...
export default function AttendancePage() {
  ...
  async function loadAttendance() {
    ...
    const { data: totalEmployees } = await supabase.from("employees").select("id").eq("active", true);
    ...
    const absent = (totalEmployees?.length || 0) - attended;

    setStats({ total: totalEmployees?.length || 0, onTime, late, absent });
    setRecords(data || []);
    setLoading(false);
  }
