...
export default function PrestamosPage() {
  ...
  const cargarDatos = async () => {
    const { data: pres, error: prestamosErr2 } = await supabase
      .from("prestamos")
      .select("*, employee:Personal(full_name, position)")
      .order("created_at", { ascending: false });
    if (prestamosErr2) console.error("Error loading prestamos:", prestamosErr2.message);
  ...
  const crearPrestamo = async () => {
    ...
    const { error: prestamosErr } = await supabase.from("prestamos").insert({
    ...
    if (prestamosErr) console.error("Error inserting prestamos:", prestamosErr.message);
