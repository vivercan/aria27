...
export default function VacacionesPage() {
  ...
  const cargarDatos = async () => {
    const { data: vac, error: vacacionesEmpleadoserr3 } = await supabase
      .from("vacaciones_empleados")
      .select("*, employee:Personal(full_name, position)")
      .eq("anio", new Date().getFullYear());
    if (vacacionesEmpleadoserr3) console.error("Error loading vacaciones_empleados:", vacacionesEmpleadoserr3.message);
    if (vac) setVacaciones(vac);

    const { data: sol, error: solicitudesVacacioneserr4 } = await supabase
      .from("solicitudes_vacaciones")
      .select("*, employee:Personal(full_name)")
      .order("created_at", { ascending: false });
    if (solicitudesVacacioneserr4) console.error("Error loading solicitudes_vacaciones:", solicitudesVacacioneserr4.message);
    if (sol) setSolicitudes(sol);

    const { data: emps, error: PersonalErr } = await supabase
      .from("Personal")
      .select("id, full_name")
      .eq("status", "ACTIVO")
      .order("full_name");
  ...
  const aprobarSolicitud = async (id: string, employee_id: string, dias: number) => {
    const { error: solicitudesVacacioneserr3 } = await supabase.from("solicitudes_vacaciones").update({ status: "APROBADA", fecha_aprobacion: new Date().toISOString() }).eq("id", id);
    if (solicitudesVacacioneserr3) console.error("Error updating solicitudes_vacaciones:", solicitudesVacacioneserr3.message);
    // Primero obtener el valor actual
    const { data: vac, error: vacacionesEmpleadoserr2 } = await supabase.from("vacaciones_empleados").select("dias_tomados").eq("employee_id", employee_id).eq("anio", new Date().getFullYear()).single();
    if (vacacionesEmpleadoserr2) console.error("Error loading vacaciones_empleados:", vacacionesEmpleadoserr2.message);
    if (vac) {
      const { error: vacacionesEmpleadoserr } = await supabase.from("vacaciones_empleados").update({ dias_tomados: vac.dias_tomados + dias }).eq("employee_id", employee_id).eq("anio", new Date().getFullYear());
      if (vacacionesEmpleadoserr) console.error("Error updating vacaciones_empleados:", vacacionesEmpleadoserr.message);
    }
    cargarDatos();
  };

  const rechazarSolicitud = async (id: string) => {
    const { error: solicitudesVacacioneserr2 } = await supabase.from("solicitudes_vacaciones").update({ status: "RECHAZADA" }).eq("id", id);
    if (solicitudesVacacioneserr2) console.error("Error updating solicitudes_vacaciones:", solicitudesVacacioneserr2.message);
    cargarDatos();
  };
  ...
  const crearSolicitud = async () => {
    ...
    const { error: solicitudesVacacioneserr } = await supabase.from("solicitudes_vacaciones").insert({
    ...
    if (solicitudesVacacioneserr) console.error("Error inserting solicitudes_vacaciones:", solicitudesVacacioneserr.message);
    setShowModal(false);
