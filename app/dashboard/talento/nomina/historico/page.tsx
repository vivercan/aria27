...
export default function HistoricoNominaPage() {
  ...
  const cargarDatos = async () => {
    setLoading(true);
    const { data, error: nominaHistoricoerr } = await supabase
      .from("nomina_historico")
      .select("*")
      .order("anio", { ascending: false })
      .order("semana", { ascending: false })
      .order("nombre");
    if (nominaHistoricoerr) console.error("Error loading nomina_historico:", nominaHistoricoerr.message);
          ...
          <table className="w-full">
            <thead className="sticky top-0 bg-slate-900/95 backdrop-blur-sm">
              <tr className="border-b border-white/10">
                ...
                <th className="text-right p-4 text-slate-400 font-medium text-sm">Rruto</th>
              ...
              {vistaAcumulado ? (
                acumuladoPorEmpleado.map((e, i) => (
                  <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02]">
                    ...
                    <td className="p-4 text-right text-purple-400">{formatMoney(e.totalTarjeta)}</td>
                ...
                registrosFiltrados.map((r, i) => (
                  <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02]">
                    ...
                    <td className="p-4 text-right text-purple-400">{formatMoney(r.pago_tarjeta)}</td>
