...
import {
  ...
  CheckSquare, Square, ChevronRight, X, Eye, Lock
} from "lucide-react";
...
export default function MisDocumentosPage() {
  ...
  const [folderType, setFolderType] = useState<"compartidos" | "privados" | null>(null);
  ...
  const loadDocs = useCallback(async () => {
    if (!selectedUser || !folderType) return;
    setLoading(true);
    const { data, error } = await supabase
      .from("mis_documentos")
      .select("*")
  ...
  function selectFolder(type: "compartidos" | "privados") {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!files || files.length === 0 || !selectedUser || !folderType) return;
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const storagePath = buildPath({
          ...
          scope: [selectedUser.id, folderType, subPath.replace(/\//g, "_")],
          file,
        });
        ...
        const { error: dbErr } = await supabase.from("mis_documentos").insert({
          ...
          owner_name: selectedUser.display_name || selectedUser.name || selectedUser.email,
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              ...
              {view === "files" && selectedUser && (
                <span className="flex items-center gap-1">
                  {userName(selectedUser)} <ChevronRight className="w-3 h-3" />
