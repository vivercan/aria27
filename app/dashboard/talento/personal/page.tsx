...
export default function HRPersonalPage() {
  ...
  async function fetchData() {
    const [empRes, centrosRes] = await Promise.all([
      supabase.from("Personal").select("*").order("employee_number", { ascending: true }),
      supabase.from("centros_trabajo").select("id, codigo, nombre").eq("activo", true)
    ]);
    if (empRes.data) setPersonal(empRes.data);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editingId) {
      await supabase.from("Personal").update(payload).eq("id", editingId);
    } else {
