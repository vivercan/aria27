...
export default function PersonalPage() {
  ...
  const cargarDatos = async () => {
    ...
    const { data: emp, error: empresasErr } = await supabase.from("empresas").select("id, nombre").order("nombre");
    if (empresasErr) console.error("Error loading empresas:", empresasErr.message);
    if (emp) setEmpresas(emp);
    const { data: ct, error: centrosTrabajoerr } = await supabase.from("centros_trabajo").select("id, nombre").eq("activo", true).order("nombre");
