...
import {
  ...
  Mail, Phone, MapPin, CreditCard, FileText, Send
} from "lucide-react";
...
IMPORTANTE: Responde SOLO con el JSON array, sin texto adicional, sin markdown, sin backticks. Solo el array JSON puro.`,
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
        ...
        <div className="fixed inset-0 z-50 flex justify-end" onClick={()=>setSelectedProduct(null)}>
          ...
          <div className="relative w-full max-w-md bg-[#0a1628] border-l border-white/[0.08] h-full overflow-auto shadow-2xl" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-4">
              ...
              ):(<div className="space-y-1.5">{productSuppliers.map((ps:any,idx:number)=>(<button key={idx} type="button" onClick={()=>{setSupplierModal(ps.suppliers);setComposerOpen(false);}} className={`w-full text-left p-2.5 rounded-lg border transition-colors ${ps.es_proveedor_preferido?"bg-emerald-500/[0.06] border-emerald-500/20 hover:bg-emerald-500/[0.10]":"bg-white/[0.02] border-white/[0.06] hover:bg-white/[0.06]"}`}>
                <div className="flex items-center gap-2">
      ...
      {/* MODAL DETALLE PROVEEDOR (B-4 22-Abr-2026) */}
      {supplierModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60" onClick={()=>{setSupplierModal(null);setComposerOpen(false);}}>
          <div className="relative w-full max-w-md bg-[#0a1628] border border-white/[0.08] rounded-xl shadow-2xl overflow-hidden" onClick={e=>e.stopPropagation()}>
            <div className="bg-gradient-to-r from-[#1E3E7A]/40 to-[#0A2450]/30 border-b border-white/[0.08] p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-sm font-bold text-white leading-tight truncate">{supplierModal.name||"Sin nombre"}</h3>
                    {supplierModal.razon_social && <p className="text-[11px] text-[#7f93b0] mt-0.5 truncate">{supplierModal.razon_social}</p>}
                  </div>
                </div>
                <button onClick={()=>{setSupplierModal(null);setComposerOpen(false);}} className="p-1.5 hover:bg-white/[0.06] rounded-lg flex-shrink-0"><X className="w-4 h-4 text-[#7f93b0]"/></button>
              </div>
            </div>

            {!composerOpen ? (
              <div className="p-4 space-y-2.5">
                <DataRow icon={MapPin} label="Ubicacion" value={[supplierModal.ciudad,supplierModal.estado].filter(Boolean).join(", ")||"-"} />
                <DataRow icon={Phone} label="Telefono" value={supplierModal.phone||"-"} />
                <DataRow icon={Phone} label="WhatsApp" value={supplierModal.whatsapp||"-"} />
                <DataRow icon={Mail} label="Correo" value={supplierModal.email||"-"} />
                <DataRow icon={CreditCard} label="Dias credito" value={supplierModal.credit_days?String(supplierModal.credit_days)+" dias":"-"} />
                ...
                <div className="pt-3 flex gap-2">
                  {supplierModal.email ? (
                    <button
                      type="button"
                      onClick={()=>{
                        const prodCtx = selectedProduct ? `${selectedProduct.sku} - ${selectedProduct.name}` : "(producto)";
                        setComposerSubject(`Cotizacion solicitada: ${prodCtx}`);
                        setComposerBody(`Estimado(a) ${supplierModal.name||"proveedor"},\n\nGrupo Constructor Urbano Avante solicita cotizacion del siguiente producto:\n\n  ${prodCtx}\n\nFavor de enviarnos:\n  - Precio unitario\n  - Tiempo de entrega\n  - Condiciones de pago\n\nGracias por su atencion.\n\n--\nDepartamento de Compras\nGrupo Constructor Urbano Avante`);
                        setComposerOpen(true);
                      }}
                      className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/40 rounded-lg text-aria-accent text-xs font-medium transition-colors"
                    >
                      <Mail className="w-3.5 h-3.5"/>Mandar correo
                    </button>
                  ) : (
                    <div className="flex-1 text-center text-[11px] text-[#4a6080] py-2">Sin email registrado</div>
                  )}
                  <button
                    type="button"
                    onClick={()=>{setSupplierModal(null);setComposerOpen(false);}}
                    className="px-3 py-2 text-xs text-[#7f93b0] hover:text-white hover:bg-white/[0.06] rounded-lg"
                  >Cerrar</button>
                </div>
              </div>
            ) : (
              <div className="p-4 space-y-3">
                <div>
                  <label className="text-[11px] text-[#7f93b0] mb-1 block flex items-center gap-1.5"><FileText className="w-3 h-3"/>Para</label>
                  <div className="px-2.5 py-1.5 bg-white/[0.04] border border-white/[0.08] rounded-lg text-xs text-white">{supplierModal.email}</div>
                </div>
                <div>
                  <label className="text-[11px] text-[#7f93b0] mb-1 block">Asunto</label>
                  <input type="text" value={composerSubject} onChange={e=>setComposerSubject(e.target.value)} className="w-full px-2.5 py-1.5 bg-white/[0.04] border border-white/[0.08] rounded-lg text-xs text-white placeholder-[#4a6080] focus:outline-none focus:border-aria-primary"/>
                </div>
                <div>
                  <label className="text-[11px] text-[#7f93b0] mb-1 block">Mensaje</label>
                  <textarea rows={9} value={composerBody} onChange={e=>setComposerBody(e.target.value)} className="w-full px-2.5 py-1.5 bg-white/[0.04] border border-white/[0.08] rounded-lg text-xs text-white placeholder-[#4a6080] focus:outline-none focus:border-aria-primary font-mono"/>
                </div>
                <div className="flex gap-2">
                  <button
                    type="button"
                    disabled={sendingMail||!composerSubject.trim()||!composerBody.trim()}
                    onClick={async ()=>{
                      setSendingMail(true);
                      try {
                        const res = await fetch(`/api/proveedores/${supplierModal.id}/correo`, {
                          method: "POST",
                        ...
                        flash("ok", "Correo enviado a " + supplierModal.email);
                        setSupplierModal(null);
                        setComposerOpen(false);
                      } catch (e:any) {
                        flash("err", "No se pudo enviar: " + (e.message||"error"));
                      } finally {
...
function DataRow({ icon: Icon, label, value }: { icon: any; label: string; value: string }) {
  return (
    <div className="flex items-start gap-2.5">
      <Icon className="w-3.5 h-3.5 text-[#7f93b0] mt-0.5 flex-shrink-0"/>
      <div className="flex-1 min-w-0">
        <p className="text-[10px] uppercase tracking-wide text-[#4a6080] font-semibold">{label}</p>
