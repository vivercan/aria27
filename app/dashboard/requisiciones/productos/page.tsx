...
export default function ProductosPage() {
    ...
    if (data) {
      ...
      if (ids.length>0) {
        ...
        if (psData) {
          ...
          psData.forEach((ps:any)=>{ if(!map[ps.product_id])map[ps.product_id]=[]; const s=ps.suppliers; if(s&&!map[ps.product_id].find((x:SupplierInfo)=>x.id===s.id))map[ps.product_id].push({id:s.id,name:s.name}); });
          setSupplierMap(map);
        }
      }
    }
  ...
  const openDetail = async (product: Product) => {
    ...
    const { data } = await supabase.from("product_suppliers")
      .select("supplier_id,precio_referencia,tiempo_entrega_dias,es_proveedor_preferido,suppliers(id,name,phone,email,ciudad,estado,whatsapp,credit_days,razon_social)")
  ...
  const exportToExcel = async (mode: "all"|"category") => {
    ...
    ws["!cols"] = [{wch:14},{wch:45},{wch:35},{wch:14},{wch:28}];
    const wb = XLSX.utils.book_new();
    const sheetName = mode==="category" && category ? category.substring(0,31) : "Catálogo completo";
    XLSX.utils.book_append_sheet(wb, ws, sheetName);
    const fileName = mode==="category" && category
      ? `catalogo_${category.replace(/\s/g,"_")}_${new Date().toISOString().split("T")[0]}.xlsx`
      : `catalogo_aria27_completo_${new Date().toISOString().split("T")[0]}.xlsx`;
        ...
        <div className="flex items-center justify-between mb-1.5">
          ...
          <div className="flex items-center gap-1.5">
            <div className="relative">
              ...
              {showExportMenu && (
                <div className="absolute right-0 top-full mt-1 bg-slate-800 border border-white/[0.1] rounded-lg shadow-xl z-20 min-w-[180px] py-1">
                  <button onClick={()=>exportToExcel("all")} className="w-full text-left px-3 py-1.5 text-xs text-white hover:bg-white/[0.08] transition-colors">Catálogo completo</button>
                  {category && <button onClick={()=>exportToExcel("category")} className="w-full text-left px-3 py-1.5 text-xs text-white hover:bg-white/[0.08] transition-colors">Solo: {category}</button>}
                </div>
              )}
            </div>
                    ...
                    <td>
                      {supps.length>0?(
                        <div className="flex gap-1 items-center">
                          {supps.slice(0,2).map(s=>(<span key={s.id} className="text-[10px] px-1 py-0.5 bg-emerald-500/10 text-emerald-400 rounded truncate max-w-[100px]" title={s.name}>{s.name.length>14?s.name.substring(0,14)+"…":s.name}</span>))}
