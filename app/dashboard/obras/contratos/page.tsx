      ...
      <div className="sticky top-0 z-10 bg-gradient-to-b from-slate-950 via-slate-950/95 to-transparent pb-4">
        ...
        <div className="mt-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-[#7f93b0] text-sm">Control de contratos de obra — anticipos, retenciones y plazos</p>
          </div>
        ...
        <div className="p-6 bg-white/[0.03] border border-white/[0.06] rounded-xl space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div><label className="text-xs text-[#7f93b0] mb-1 block">Obra *</label>
              <input required value={form.obra_nombre} onChange={e => setForm({...form, obra_nombre: e.target.value})} placeholder="Nombre de la obra" className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm placeholder:text-[#4a6080] focus:outline-none" /></div>
            <div><label className="text-xs text-[#7f93b0] mb-1 block">Cliente *</label>
              <input required value={form.cliente} onChange={e => setForm({...form, cliente: e.target.value})} placeholder="Razón social" className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm placeholder:text-[#4a6080] focus:outline-none" /></div>
            <div><label className="text-xs text-[#7f93b0] mb-1 block">RFC Cliente</label>
              <input value={form.rfc_cliente} onChange={e => setForm({...form, rfc_cliente: e.target.value.toUpperCase()})} placeholder="RFC" maxLength={13} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm placeholder:text-[#4a6080] focus:outline-none uppercase" /></div>
            ...
            <div><label className="text-xs text-[#7f93b0] mb-1 block">Monto del Contrato *</label>
              <input type="number" required min="0.01" step="0.01" value={form.monto_contrato} onChange={e => setForm({...form, monto_contrato: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm focus:outline-none" /></div>
            <div><label className="text-xs text-[#7f93b0] mb-1 block">% Anticipo *</label>
              <input type="number" required min="0" max="100" step="0.01" value={form.anticipo_porcentaje} onChange={e => setForm({...form, anticipo_porcentaje: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm focus:outline-none" /></div>
            <div><label className="text-xs text-[#7f93b0] mb-1 block">% Retención Garantía *</label>
              <input type="number" required min="0" max="100" step="0.01" value={form.retencion_porcentaje} onChange={e => setForm({...form, retencion_porcentaje: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm focus:outline-none" /></div>
            <div><label className="text-xs text-[#7f93b0] mb-1 block">Fecha Inicio *</label>
              <input type="date" required value={form.fecha_inicio} onChange={e => setForm({...form, fecha_inicio: e.target.value})} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm focus:outline-none" /></div>
            <div><label className="text-xs text-[#7f93b0] mb-1 block">Fecha Fin *</label>
              <input type="date" required value={form.fecha_fin} onChange={e => setForm({...form, fecha_fin: e.target.value})} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm focus:outline-none" /></div>
            <div className="md:col-span-3"><label className="text-xs text-[#7f93b0] mb-1 block">Descripción</label>
              <input value={form.descripcion} onChange={e => setForm({...form, descripcion: e.target.value})} placeholder="Descripción del contrato" className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm placeholder:text-[#4a6080] focus:outline-none" /></div>
          </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="p-8 text-center text-[#7f93b0]">Sin contratos registrados</td></tr>
              ) : filtered.map(c => (
