  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[#7f93b0] text-sm">Colados, pruebas de resistencia y gestión de cilindros de prueba</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-br from-aria-accent/10 to-aria-primary/10 border border-aria-accent/20 hover:border-aria-accent/40 transition-all">
          <div className="flex items-start justify-between mb-3">
            <div>
              <p className="text-[#7f93b0] text-sm mb-1">Total m³ Colados</p>
          ...
          <p className="text-xs text-[#4a6080]">Volumen total de concreto colado</p>
        </div>
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-br from-aria-primary/10 to-indigo-500/10 border border-aria-primary/20 hover:border-aria-primary/40 transition-all">
          <div className="flex items-start justify-between mb-3">
            <div>
              <p className="text-[#7f93b0] text-sm mb-1">Remisiones</p>
          ...
          <p className="text-xs text-[#4a6080]">Colados registrados</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        <label className="text-sm text-[#7f93b0]">Filtrar por obra:</label>
        <select
          value={filtroObra}
          onChange={e => setFiltroObra(e.target.value)}
          className="px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white focus:outline-none focus:border-aria-accent/50"
        >
        ...
        {filtroObra && (
          <button
            ...
            className="text-xs text-[#7f93b0] hover:text-[#c9d8ed] underline"
          >
            Limpiar filtro
          </button>
        )}
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-[rgba(4,8,16,0.98)] backdrop-blur z-10">
                  <tr className="border-b border-white/[0.08]">
                    <th className="text-left p-4 text-[#7f93b0] text-xs font-medium">Obra</th>
                    <th className="text-right p-4 text-[#7f93b0] text-xs font-medium">Remisiones</th>
                    <th className="text-right p-4 text-[#7f93b0] text-xs font-medium">m³ Total</th>
                    <th className="text-right p-4 text-[#7f93b0] text-xs font-medium">Costo Total</th>
                    <th className="text-center p-4 text-[#7f93b0] text-xs font-medium">f'c Promedio</th>
                    <th className="text-center p-4 text-[#7f93b0] text-xs font-medium">Cilindros</th>
                    <th className="text-center p-4 text-[#7f93b0] text-xs font-medium">% Cumple</th>
                  </tr>
                </thead>
                <tbody>
                  {resumenPorObra.map(resumen => (
                    ...
                    >
                      <td className="p-4 text-white font-medium flex items-center gap-2">
                        {resumen.obra}
                        <ChevronRight className="w-4 h-4 text-[#4a6080]" />
                      </td>
          ...
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden">
            <div className="overflow-auto max-h-[400px]">
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-[rgba(4,8,16,0.98)] backdrop-blur z-10">
                  <tr className="border-b border-white/[0.08]">
                    <th className="text-left p-4 text-[#7f93b0] text-xs font-medium">Fecha</th>
                    <th className="text-left p-4 text-[#7f93b0] text-xs font-medium">Obra</th>
                    <th className="text-left p-4 text-[#7f93b0] text-xs font-medium">Proveedor</th>
                    <th className="text-right p-4 text-[#7f93b0] text-xs font-medium">m³</th>
                    <th className="text-center p-4 text-[#7f93b0] text-xs font-medium">f'c</th>
                    <th className="text-right p-4 text-[#7f93b0] text-xs font-medium">Costo</th>
                    <th className="text-center p-4 text-[#7f93b0] text-xs font-medium">Cilindros</th>
                  </tr>
                </thead>
                <tbody>
                  {ultimasRemisiones.map(rem => {
                    ...
                    return (
                      <tr key={rem.id} className="border-b border-white/[0.05] hover:bg-white/[0.02]">
                        <td className="p-4 text-[#c9d8ed] text-xs">{new Date(rem.fecha_colado).toLocaleDateString("es-MX")}</td>
      ...
      {remisiones.length === 0 && (
        <div className="p-12 rounded-2xl bg-white/[0.02] border border-white/[0.08] text-center">
          ...
          <p className="text-[#7f93b0] mb-4">No hay remisiones de concreto registradas aún</p>
