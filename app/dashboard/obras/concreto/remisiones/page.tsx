    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {[
          ...
          { l: "Costo total", v: fmtMoney(stats.costoTotal, { noDecimals: true }), c: "text-aria-accent" },
          { l: "Pruebas OK", v: stats.pruebasOk, c: "text-aria-accent" },
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Buscar por obra, número o elemento..." value={search} onChange={e => setSearch(e.target.value)}
          className="w-full pl-10 pr-4 py-2 bg-[#0c1d38]/50 border border-white/[0.08] rounded-lg text-white placeholder-[#4a6080] focus:outline-none focus:border-aria-accent" />
      </div>

      {loading ? (
        <div className="flex justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-aria-accent" /></div>
      ) : filtradas.length === 0 ? (
        <div className="text-center py-12 text-[#7f93b0]">No hay remisiones registradas.</div>
      ) : (
        <div className="space-y-4">
          {filtradas.map(r => {
            const cils = cilindrosPorRem(r.id);
            return (
              <div key={r.id} className="rounded-xl bg-[#0c1d38]/50 border border-white/[0.05] p-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <span className="text-white font-bold text-lg">{r.numero_remision}</span>
                      <span className="text-xs text-[#7f93b0]">{r.fecha_colado ? new Date(r.fecha_colado).toLocaleDateString("es-MX") : "—"}</span>
                    </div>
                    <div className="text-sm text-[#c9d8ed]">{r.obra} · {r.elemento || "—"} · {r.proveedor || "—"}</div>
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/[0.05] flex-wrap gap-2">
                  <div className="flex gap-2 flex-wrap">
                    {cils.length === 0 ? (
                      <span className="text-xs text-[#4a6080]">Sin pruebas</span>
                    ) : cils.map(c => (
                      <div key={c.id} className={`flex items-center gap-1 px-2 py-1 rounded text-xs border ${c.cumple ? "bg-emerald-500/10 text-aria-accent border-emerald-500/40" : "bg-rose-500/10 text-rose-300 border-rose-500/40"}`}>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => abrirEditarRem(r)} className="p-1.5 bg-white/[0.05] hover:bg-[#0f2448] rounded"><Edit2 className="w-4 h-4 text-white" /></button>
          ...
          <div className="bg-[#0a1628] border border-white/[0.08] rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label className="text-sm text-[#7f93b0] mb-1 block">Fecha colado *</label><input type="date" value={formRem.fecha_colado} onChange={e => setFormRem({ ...formRem, fecha_colado: e.target.value })} className="w-full px-3 py-2 bg-[#0c1d38] border border-white/[0.08] rounded-lg text-white focus:outline-none focus:border-aria-accent" />{formRemErrors.fecha_colado && <p className="text-red-400 text-xs mt-1">{formRemErrors.fecha_colado}</p>}</div>
                <div><label className="text-sm text-[#7f93b0] mb-1 block">Resistencia f'c</label><input type="text" value={formRem.resistencia_fc} onChange={e => setFormRem({ ...formRem, resistencia_fc: e.target.value })} className="w-full px-3 py-2 bg-[#0c1d38] border border-white/[0.08] rounded-lg text-white font-mono focus:outline-none focus:border-aria-accent" /></div>
                <div><label className="text-sm text-[#7f93b0] mb-1 block">Revenimiento (cm)</label><input type="number" min="0" step="0.5" value={formRem.revenimiento} onChange={e => setFormRem({ ...formRem, revenimiento: e.target.value })} className="w-full px-3 py-2 bg-[#0c1d38] border border-white/[0.08] rounded-lg text-white focus:outline-none focus:border-aria-accent" /></div>
                <div><label className="text-sm text-[#7f93b0] mb-1 block">m³ *</label><input type="number" min="0" step="0.5" value={formRem.m3} onChange={e => setFormRem({ ...formRem, m3: e.target.value })} className="w-full px-3 py-2 bg-[#0c1d38] border border-white/[0.08] rounded-lg text-white focus:outline-none focus:border-aria-accent" />{formRemErrors.m3 && <p className="text-red-400 text-xs mt-1">{formRemErrors.m3}</p>}</div>
                <div><label className="text-sm text-[#7f93b0] mb-1 block">Elemento</label><input type="text" value={formRem.elemento} onChange={e => setFormRem({ ...formRem, elemento: e.target.value })} placeholder="Losa N1, Castillo C-1, Zapata Z-3..." className="w-full px-3 py-2 bg-[#0c1d38] border border-white/[0.08] rounded-lg text-white focus:outline-none focus:border-aria-accent" /></div>
                <div><label className="text-sm text-[#7f93b0] mb-1 block">Temperatura (°C)</label><input type="number" min="0" value={formRem.temperatura} onChange={e => setFormRem({ ...formRem, temperatura: e.target.value })} className="w-full px-3 py-2 bg-[#0c1d38] border border-white/[0.08] rounded-lg text-white focus:outline-none focus:border-aria-accent" /></div>
              ...
              <div className="text-sm text-[#7f93b0]">Total estimado: <span className="text-aria-accent font-bold">{fmtMoney((Number(formRem.m3) || 0) * (Number(formRem.costo_unitario) || 0), { noDecimals: true })}</span></div>
            </div>
      ...
      {showFormCil && remActiva && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-[#0a1628] border border-white/[0.08] rounded-2xl max-w-xl w-full">
            <div className="p-5 border-b border-white/[0.08] flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-white">Prueba de cilindro</h2>
                <p className="text-sm text-[#7f93b0]">Remisión {remActiva.numero_remision} · {remActiva.resistencia_fc}</p>
              </div>
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label className="text-sm text-[#7f93b0] mb-1 block">Resistencia (kg/cm²) *</label><input type="number" min="0" value={formCil.resistencia_alcanzada} onChange={e => setFormCil({ ...formCil, resistencia_alcanzada: e.target.value })} className="w-full px-3 py-2 bg-[#0c1d38] border border-white/[0.08] rounded-lg text-white focus:outline-none focus:border-aria-accent" />{formCilErrors.resistencia_alcanzada && <p className="text-red-400 text-xs mt-1">{formCilErrors.resistencia_alcanzada}</p>}</div>
                <div className="col-span-2"><label className="text-sm text-[#7f93b0] mb-1 block">Laboratorio</label><input type="text" value={formCil.laboratorio} onChange={e => setFormCil({ ...formCil, laboratorio: e.target.value })} className="w-full px-3 py-2 bg-[#0c1d38] border border-white/[0.08] rounded-lg text-white focus:outline-none focus:border-aria-accent" /></div>
            ...
            <div className="p-5 border-t border-white/[0.08] flex justify-end gap-3">
              <button onClick={() => { setShowFormCil(false); setRemActiva(null); setFormCil(EMPTY_CIL); }} className="px-4 py-2 bg-[#0f2448] hover:bg-[#162040] text-white rounded-lg">Cancelar</button>
