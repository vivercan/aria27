    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <button onClick={() => { setEditando(null); setFormRem(EMPTY_REM); setShowFormRem(true); }}
          className="px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-lg flex items-center gap-2 font-medium">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          { l: "m³ totales", v: stats.totalM3.toLocaleString("es-MX"), c: "text-sky-300" },
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Buscar por obra, número o elemento..." value={search} onChange={e => setSearch(e.target.value)}
          className="w-full pl-10 pr-4 py-2 bg-slate-800/50 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:border-sky-500" />
      </div>
              ...
              <div key={r.id} className="rounded-xl bg-slate-800/50 border border-slate-700/50 p-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-sky-300">{r.m3} m³</div>
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-700/50 flex-wrap gap-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setRemActiva(r); setFormCil(EMPTY_CIL); setShowFormCil(true); }}
                      className="px-3 py-1 text-xs bg-sky-600/80 hover:bg-sky-600 text-white rounded flex items-center gap-1">
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              <div><label className="text-sm text-slate-400 mb-1 block">Observaciones</label><textarea value={formRem.observaciones} onChange={e => setFormRem({ ...formRem, observaciones: e.target.value })} rows={2} className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white focus:outline-none focus:border-sky-500" /></div>
            ...
            <div className="sticky bottom-0 bg-slate-900 border-t border-slate-700 p-5 flex justify-end gap-3">
              ...
              <button onClick={guardarRem} disabled={guardando} className="px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-lg flex items-center gap-2 disabled:opacity-50">{guardando ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}Guardar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-xl w-full">
            ...
            <div className="p-5 border-t border-slate-700 flex justify-end gap-3">
              ...
              <button onClick={guardarCil} disabled={guardando} className="px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-lg flex items-center gap-2 disabled:opacity-50">{guardando ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}Guardar prueba</button>
            </div>
          </div>
        </div>
      )}
