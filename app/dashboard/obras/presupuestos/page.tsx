...
import { ArrowLeft, Plus, Search, DollarSign, BarChart3, TrendingUp, AlertTriangle, Layers } from "lucide-react";
...
export default function PresupuestosPage() {
  ...
  async function loadData() {
    try {
      const { data, error: presupuestosPartidaserr } = await supabase.from("presupuestos_partidas").select("*").order("obra_nombre").order("clave");
      if (presupuestosPartidaserr) console.error("Error loading presupuestos_partidas:", presupuestosPartidaserr.message);
      setPartidas(data || []);
