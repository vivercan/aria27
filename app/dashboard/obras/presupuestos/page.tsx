  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="sticky top-0 z-10 bg-gradient-to-b from-slate-950 via-slate-950/95 to-transparent pb-4">
        ...
        <div className="mt-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-[#7f93b0] text-sm">Catálogo de partidas con precios unitarios por obra</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          {porCategoria.map((cat, i) => (
            <div key={i} className="p-3 bg-white/[0.02] border border-white/[0.05] rounded-lg">
              ...
              <p className="text-xs text-[#4a6080]">{cat.partidas} partidas · {totalPresupuesto > 0 ? ((cat.total / totalPresupuesto) * 100).toFixed(1) : 0}%</p>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="p-6 bg-white/[0.03] border border-white/[0.06] rounded-xl space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div><label className="text-xs text-[#7f93b0] mb-1 block">Obra *</label>
              <select value={form.obra_nombre} onChange={e => setForm({...form, obra_nombre: e.target.value})} required className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm focus:outline-none">
                <option value="">-- Selecciona obra del catálogo --</option>
                {obrasCat.map(o => <option key={o.id} value={o.nombre}>{o.nombre}</option>)}
              </select></div>
            <div><label className="text-xs text-[#7f93b0] mb-1 block">Clave</label>
              <input value={form.clave} onChange={e => setForm({...form, clave: e.target.value.toUpperCase()})} placeholder="Ej: ALB-001" className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm placeholder:text-[#4a6080] focus:outline-none" /></div>
            <div><label className="text-xs text-[#7f93b0] mb-1 block">Categoría</label>
              <select value={form.categoria} onChange={e => setForm({...form, categoria: e.target.value})} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm focus:outline-none">
            ...
            <div><label className="text-xs text-[#7f93b0] mb-1 block">Unidad</label>
              <select value={form.unidad} onChange={e => setForm({...form, unidad: e.target.value})} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm focus:outline-none">
            ...
            <div className="md:col-span-2"><label className="text-xs text-[#7f93b0] mb-1 block">Descripción *</label>
              <input value={form.descripcion} onChange={e => setForm({...form, descripcion: e.target.value})} placeholder="Descripción de la partida" required className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm placeholder:text-[#4a6080] focus:outline-none" /></div>
            <div><label className="text-xs text-[#7f93b0] mb-1 block">Cantidad *</label>
              <input type="number" min="0.01" step="0.01" required value={form.cantidad} onChange={e => setForm({...form, cantidad: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm focus:outline-none" /></div>
            <div><label className="text-xs text-[#7f93b0] mb-1 block">Precio Unitario *</label>
              <input type="number" min="0.01" step="0.01" required value={form.precio_unitario} onChange={e => setForm({...form, precio_unitario: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm focus:outline-none" /></div>
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select value={filterObra} onChange={e => setFilterObra(e.target.value)} className="px-4 py-2 bg-white/[0.04] border border-white/[0.08] rounded-xl text-white text-sm focus:outline-none">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="p-8 text-center text-[#7f93b0]">Sin partidas registradas</td></tr>
              ) : filtered.map(p => (
                <tr key={p.id} className="border-t border-white/[0.05] hover:bg-white/[0.02]">
                  ...
                  <td className="p-3 text-right text-[#c9d8ed]">${(p.precio_unitario || 0).toLocaleString()}</td>
