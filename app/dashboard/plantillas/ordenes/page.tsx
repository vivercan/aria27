  ...
  return (
    <div className="flex flex-col gap-6 p-6 h-full overflow-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Formatos de Ãrdenes de Compra</h1>
          <p className="text-slate-400 text-sm">GestiÃ³n de formatos y plantillas de Ã³rdenes de compra</p>
        </div>
          ...
          <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-lg" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <input value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value })} className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white text-sm focus:border-blue-500/50 focus:outline-none" placeholder="DescripciÃ³n de la orden" />
              </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="p-4 bg-white/[0.03] border border-white/[0.06] rounded-xl">
          ...
          <p className="text-xs text-slate-400">Total Ãrdenes</p>
        </div>
        <div className="p-4 bg-white/[0.03] border border-white/[0.06] rounded-xl">
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por nombre, nÃºmero o proveedor..."
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="p-8 text-center text-slate-400">
                  {ordenes.length === 0 ? "No hay formatos de Ã³rdenes registrados." : "No se encontraron resultados."}
                </td></tr>
              ) : filtered.map(o => (
                <tr key={o.id} className="border-t border-white/5 hover:bg-white/[0.02]">
                  <td className="p-3 text-blue-400 font-mono text-xs">{o.numero || "â"}</td>
                  <td className="p-3 text-white font-medium">{o.nombre}</td>
                  <td className="p-3 text-slate-300">{o.proveedor || "â"}</td>
                  <td className="p-3 text-slate-400">{o.obra || "â"}</td>
