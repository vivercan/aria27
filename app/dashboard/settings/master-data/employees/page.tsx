...
export default function EmployeesPage() {
  ...
  async function loadData() {
    ...
    const [empRes, cenRes] = await Promise.all([
      supabase.from("employees").select("*, work_centers(name)").order("name"),
  ...
  async function handleSave() {
    ...
    if (editingId) {
      await supabase.from("employees").update(data).eq("id", editingId);
    } else {
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 overflow-hidden">
        ...
        {employees.length === 0 && <div className="p-8 text-center text-slate-400">No hay empleados registrados</div>}
      </div>
