        ...
        <div className="flex-none mx-6 mt-3 p-5 bg-white/[0.03] border border-white/[0.06] rounded-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="md:col-span-2">
              <label className="text-xs text-[#7f93b0] mb-1 block">Nombre / razón social *</label>
            ...
            <div>
              <label className="text-xs text-[#7f93b0] mb-1 block">RFC</label>
              <input value={form.rfc} onChange={e => setForm({ ...form, rfc: e.target.value.toUpperCase() })} maxLength={13} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm uppercase" />
            </div>
            <div>
              <label className="text-xs text-[#7f93b0] mb-1 block">Contacto principal</label>
              <input value={form.contacto} onChange={e => setForm({ ...form, contacto: e.target.value })} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm" />
            </div>
            <div>
              <label className="text-xs text-[#7f93b0] mb-1 block">Teléfono</label>
              <input value={form.telefono} onChange={e => setForm({ ...form, telefono: e.target.value })} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm" />
            </div>
            <div>
              <label className="text-xs text-[#7f93b0] mb-1 block">Email</label>
              <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm" />
            </div>
            <div>
              <label className="text-xs text-[#7f93b0] mb-1 block">Días de crédito</label>
              <input type="number" min="0" value={form.dias_credito} onChange={e => setForm({ ...form, dias_credito: e.target.value })} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm" />
            ...
            <div>
              ...
              <select value={form.estatus} onChange={e => setForm({ ...form, estatus: e.target.value })} className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-white text-sm">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={c.id} className={`border-t border-white/[0.05] hover:bg-white/[0.02] ${c.estatus === "INACTIVO" ? "opacity-60" : ""}`}>
                  <td className="p-3">
                    <p className="text-white font-medium">{c.nombre}</p>
                    {c.observaciones && <p className="text-xs text-[#4a6080] truncate max-w-xs">{c.observaciones}</p>}
                  </td>
                  <td className="p-3 text-[#c9d8ed] font-mono text-xs">{c.rfc || "—"}</td>
                  <td className="p-3 text-[#c9d8ed]">{c.contacto || "—"}</td>
