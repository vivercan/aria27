...
export default function PorPagarPage() {
  ...
  async function loadData() {
    try {
      const { data: ocs, error: purchaseOrderserr2 } = await supabase.from("purchase_orders").select("*").order("created_at", { ascending: false });
      if (purchaseOrderserr2) console.error("Error loading purchase_orders:", purchaseOrderserr2.message);
      const hoy = new Date();
  ...
  async function confirmarPago() {
    ...
    const { error: purchaseOrderserr } = await supabase.from("purchase_orders").update({ monto_pagado: nuevoPagado }).eq("id", pagoModal.id);
    if (purchaseOrderserr) console.error("Error updating purchase_orders:", purchaseOrderserr.message);
    setPagoSaving(false);
    setPagoModal(null);
    loadData();
  }
