...
export default function CosteoPage() {
  ...
  async function loadData() {
    try {
      ...
      const { data: centers, error: centrosTrabajoerr } = await supabase.from("centros_trabajo").select("id, code:codigo, name:nombre, active:activo, budget:presupuesto, client:cliente, status:estado, start_date:fecha_inicio, end_date:fecha_fin, description:descripcion, location:direccion");
      ...
      const { data: ocs, error: purchaseOrderserr } = await supabase.from("purchase_orders").select("obra_nombre, total, categoria");
      if (purchaseOrderserr) console.error("Error loading purchase_orders:", purchaseOrderserr.message);

      // Cargar gastos de obra
      const { data: gastos, error: gastosErr } = await supabase.from("gastos").select("obra, monto, categoria");
      if (gastosErr) console.error("Error loading gastos:", gastosErr.message);
