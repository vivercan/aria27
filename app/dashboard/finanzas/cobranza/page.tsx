...
export default function CobranzaPage() {
  ...
  async function guardar() {
    ...
    const { count, error: estimacionesErr2 } = await supabase.from("estimaciones").select("*", { count: "exact", head: true }).eq("obra_nombre", form.obra_nombre);
    if (estimacionesErr2) console.error("Error loading estimaciones:", estimacionesErr2.message);
  ...
  async function confirmarCobro() {
    ...
    setCobroSaving(true);
    const { error: estimacionesErr } = await supabase.from("estimaciones").update({
      monto_cobrado: montoCobrado,
      status: "COBRADA",
      fecha_cobro: new Date().toISOString().split("T")[0],
    }).eq("id", cobroModal.id);
    if (estimacionesErr) console.error("Error updating estimaciones:", estimacionesErr.message);
